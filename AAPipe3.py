...
numSeqs = int(input("Enter number of sequences: "))
#numSeqs = int(numSeqs.strip())
...
def extract_amino_acids(input_file):
    ...
    for record in SeqIO.parse(input_file, "fasta"):
        ...
        aa_percentages['id'] = record.id  # add protein ID to the dictionary
...
# Write amino acid frequencies per protein to output file
def write_csv2(output_file2, data):
    with open(output_file2, "w", newline="") as csvfile:
        ...
        for aa in data:
            for k, v in aa.items():
                if k != 'id':
                    writer.writerow([aa['id'], k, v])
                    
# Write amino acid frequencies per protein to output file
write_csv2(output_file2, amino_acids)

aa_outfile = open("test.outs.csv", "w", newline="")
